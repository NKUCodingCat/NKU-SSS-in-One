...
class RSA_password():
	def __init__(self):
		self.Func_in_js = """(function shit(s) {
		//===============
		var Fuck = function($w) {
			...
			RSAUtils.biFromDecimal = function(s) {
				...
				if (i == s.length) {
					...
					if (fgl == 0) fgl = dpl10;
					result = RSAUtils.biFromNumber(Number(s.substring(i, i+fgl)));
					i += fgl;
					while (i < s.length) {
						result = RSAUtils.biAdd(RSAUtils.biMultiply(result, lr10), RSAUtils.biFromNumber(Number(s.substring(i, i+dpl10))));
						i += dpl10;
					}
					result.isNeg = isNeg;
				}
			...
			RSAUtils.biFromHex = function(s) {
				...
				for (var i = sl,
				j = 0; i > 0; i -= 4, ++j) {
					result.digits[j] = RSAUtils.hexToDigit(s.substring(Math.max(i - 4, 0), Math.max(i - 4, 0)+Math.min(i, 4)));
				}
				return result;
			};
		...
		*/
		...
		var exponent = "010001";
		var key = rsa_obj.RSAUtils.getKeyPair(exponent,"", modulus);
		return rsa_obj.RSAUtils.encryptedString(key, pass);
		}
		var new_pass = Enc(Fuck(new Object()), s);
		...
		self.ctxt.enter()
		self.Func = self.ctxt.eval(self.Func_in_js)
	
	def RSA(self, password):
		return self.Func.call("shit", password)